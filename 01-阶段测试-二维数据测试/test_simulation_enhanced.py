...
class EnhancedRobotEnv:
    """简化机器人环境"""
    def __init__(self, action_scale=0.02):
    ...
    def step(self, action):
        ...
        self.state[:3] += action[:3] * self.action_scale
        self.state[3:6] += action[3:6] * self.action_scale
...
def test_enhanced_control():
    ...
    # 创建环境
    print("\n创建模拟环境...")
    env = EnhancedRobotEnv(action_scale=0.02)
    print("✅ 环境创建成功\n")
    ...
    instruction = instructions[episode % 4]
    instructions = ["move to target", "reach goal", "navigate to target", "go to position"]
