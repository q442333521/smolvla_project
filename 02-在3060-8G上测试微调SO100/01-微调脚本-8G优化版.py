...
class Config:
    ...
    # 优化配置
    use_amp = True              # 混合精度
    freeze_vlm = False           # 冻结VLM，只训练Action Expert
...
def main():
    ...
    # 2. 创建数据加载器
    # 定义collate函数来重映射图像key并添加语言token
    def collate_fn(batch_list):
        ...
        if "observation.language.tokens" not in batch:
            # 简单的token化（使用任务字段）
            task_text = batch.get("task", ["Pick up the cube and place it in the box."] * len(batch_list))
            if isinstance(task_text[0], str):
                # 假设已有tokenizer，这里用占位符
                batch["observation.language.tokens"] = torch.zeros((len(batch_list), 77), dtype=torch.long)
                batch["observation.language.attention_mask"] = torch.ones((len(batch_list), 77), dtype=torch.long)
        return batch
