...
class SimpleRobotEnv:
    ...
    def _get_observation(self):
        ...
        img1 = transform(img).cuda().float()
        img2 = transform(F.rotate(img, 15)).cuda().float()
        img3 = transform(F.hflip(img)).cuda().float()
        return {
            "image1": img1,
            "image2": img2,
            "image3": img3,
    ...
    def step(self, action):
        ...
        done = distance < 0.08 or self.current_step >= self.max_steps
...
def test_closed_loop_control():
    ...
    for episode in range(num_episodes):
        ...
        observation = {
            "observation.images.camera1": obs["image1"].unsqueeze(0),  # (1, 3, 256, 256)
            "observation.images.camera2": obs["image2"].unsqueeze(0),
            "observation.images.camera3": obs["image3"].unsqueeze(0),
            "observation.images.camera2": obs["image1"].unsqueeze(0),  # (1, 3, 256, 256)
            "observation.images.camera3": obs["image1"].unsqueeze(0),  # (1, 3, 256, 256)
        ...
        while not done and step_count < 100:
            ...
            observation["observation.images.camera1"] = obs["image1"].unsqueeze(0)
            observation["observation.images.camera2"] = obs["image1"].unsqueeze(0)
            observation["observation.images.camera3"] = obs["image1"].unsqueeze(0)
