...
def download_lerobot_dataset(dataset_name, save_dir="~/smolvla_project/datasets"):
    ...
    Args:
        dataset_name: 数据集名称，如 "lerobot/pusht"
        save_dir: 保存目录
    
    Returns:
        dataset_path: 下载后的数据集路径
    """
    print(f"\n{'='*60}")
...
def main():
    ...
    """
    print("\n" + "🚀"*30)
    print("LeRobot数据集下载和测试")
    print("🚀"*30)
    
    # 推荐的测试数据集（从小到大）
    test_datasets = [
        "lerobot/pusht",                          # 最小，最容易下载
        "lerobot/aloha_sim_insertion_human",      # 中等大小
        # "lerobot/metaworld_mt50",               # 较大，可选
    ]
    ...
    # 保存结果到文件
    result_file = Path("~/smolvla_project").expanduser() / "dataset_test_results.json"
